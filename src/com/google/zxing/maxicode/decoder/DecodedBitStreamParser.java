...
final class DecodedBitStreamParser {
	...
	private static final String[] SETS = {
			"\nABCDEFGHIJKLMNOPQRSTUVWXYZ" + ECI + FS + GS + RS + NS + ' ' + PAD + "\"#$%&'()*+,-./0123456789:" + SHIFTB
					+ SHIFTC + SHIFTD + SHIFTE + LATCHB,
			"`abcdefghijklmnopqrstuvwxyz" + ECI + FS + GS + RS + NS + '{' + PAD + "}~\u007F;<=>?[\\]^_ ,./:@!|" + PAD
					+ TWOSHIFTA + THREESHIFTA + PAD + SHIFTA + SHIFTC + SHIFTD + SHIFTE + LATCHA,
	...
	static DecoderResult decode(byte[] bytes, int mode) {
		...
		case 3:
			...
			if (mode == 2) {
				...
				postcode = DecodedBitStreamParser.decimalFormat000(getPostCode2Length(bytes), pc);
			} else {
				postcode = getPostCode3(bytes);
			}
			//NumberFormat threeDigits = new DecimalFormat("000");
			String country = DecodedBitStreamParser.decimalFormat000(3, getCountry(bytes)); //threeDigits.format(getCountry(bytes));
			String service = DecodedBitStreamParser.decimalFormat000(3, getServiceClass(bytes)); //threeDigits.format(getServiceClass(bytes));
	...
	private static int getPostCode2(byte[] bytes) {
		return getInt(bytes, new byte[] { 33, 34, 35, 36, 25, 26, 27, 28, 29, 30, 19, 20, 21, 22, 23, 24, 13, 14, 15,
				16, 17, 18, 7, 8, 9, 10, 11, 12, 1, 2 });
	}

	private static String getPostCode3(byte[] bytes) {
		return String.valueOf(new char[] { SETS[0].charAt(getInt(bytes, new byte[] { 39, 40, 41, 42, 31, 32 })),
	...
	private static String getMessage(byte[] bytes, int start, int len) {
		...
		for (int i = start; i < start + len; i++) {
			...
			case THREESHIFTA:
				...
				break;
			case NS:
				int nsval = (bytes[++i] << 24) + (bytes[++i] << 18) + (bytes[++i] << 12) + (bytes[++i] << 6)
						+ bytes[++i];
				//sb.append(new DecimalFormat("000000000").format(nsval));
				sb.append(DecodedBitStreamParser.decimalFormat000(9, nsval));
				break;
			case LOCK:
				shift = -1;
				break;
	...
	/**
	 * Supports only positive value, fill always with 0
	 * @param length
	 * @param val
	 * @return
	 */
	public static String decimalFormat000(int length, int val) {
		int numDigits = 0;
		...
		char[] padding = new char[length - numDigits];
