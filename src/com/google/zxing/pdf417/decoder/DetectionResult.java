...
final class DetectionResult {
	...
	// we should be able to estimate the row height and use it as a hint for the row
	// number
	// we should also fill the rows top to bottom and bottom to top
	/**
	 * @return number of codewords which don't have a valid row number. Note that
	 *         the count is not accurate as codewords will be counted several times.
	 *         It just serves as an indicator to see when we can stop adjusting row
	 *         numbers
	 */
	...
	private int adjustRowNumbersByRow() {
		...
		// TODO we should only do full row adjustments if row numbers of left and right
		// row indicator column match.
		// Maybe it's even better to calculated the height (in codeword rows) and divide
		// it by the number of barcode
		// rows. This, together with the LRI and RRI row numbers should allow us to get
		// a good estimate where a row
	...
	public String toString() {
		...
		for (int codewordsRow = 0; codewordsRow < rowIndicatorColumn.getCodewords().length; codewordsRow++) {
			sb.append("CW ");
			sb.append(codewordsRow >= 1000 ? "" + codewordsRow : codewordsRow >= 100 ? " " + codewordsRow : codewordsRow >= 10 ? "  " + codewordsRow : "   " + codewordsRow);
			//sb.format("CW %3d:", codewordsRow);
			for (int barcodeColumn = 0; barcodeColumn < barcodeColumnCount + 2; barcodeColumn++) {
				...
				//sb.format(" %3d|%3d", codeword.getRowNumber(), codeword.getValue());
				int j = codeword.getRowNumber();
				int i = codeword.getValue();
