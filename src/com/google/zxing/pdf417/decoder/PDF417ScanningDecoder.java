...
public final class PDF417ScanningDecoder {
	...
	// This approach also allows to detect more details about the barcode, e.g. if a
	// bar type (white or black) is wider
	...
	public static DecoderResult decode(BitMatrix image, ResultPoint imageTopLeft, ResultPoint imageBottomLeft,
		...
		BoundingBox boundingBox = new BoundingBox(image, imageTopLeft, imageBottomLeft, imageTopRight,
				imageBottomRight);
		...
		for (boolean firstPass = true;; firstPass = false) {
			...
			if (imageTopRight != null) {
				rightRowIndicatorColumn = getRowIndicatorColumn(image, boundingBox, imageTopRight, false,
		...
		for (int barcodeColumnCount = 1; barcodeColumnCount <= maxBarcodeColumn; barcodeColumnCount++) {
			...
			if (detectionResult.getDetectionResultColumn(barcodeColumn) != null) {
				// This will be the case for the opposite row indicator column, which doesn't
			...
			// TODO start at a row for which we know the start position, then detect upwards
	...
	private static DetectionResult merge(DetectionResultRowIndicatorColumn leftRowIndicatorColumn,
			DetectionResultRowIndicatorColumn rightRowIndicatorColumn) throws NotFoundException {
	...
	private static DetectionResultRowIndicatorColumn getRowIndicatorColumn(BitMatrix image, BoundingBox boundingBox,
			ResultPoint startPoint, boolean leftToRight, int minCodewordWidth, int maxCodewordWidth) {
	...
	private static void adjustCodewordCount(DetectionResult detectionResult, BarcodeValue[][] barcodeMatrix)
		...
		if (numberOfCodewords.length == 0) {
			...
			// The calculated one is more reliable as it is derived from the row indicator
			// columns
			barcodeMatrix01.setValue(calculatedNumberOfCodewords);
		}
	}
	 ...
	 * yield a single most likely value. The current error correction implementation
	 * doesn't deal with erasures very well, so it's better to provide a value for
	 * these ambiguous codewords instead of treating it as an erasure. The problem
	 * is that we don't know which of the ambiguous values to choose. We try decode
	 * using the first value, and if that fails, we use another of the ambiguous
	 * values and try to decode again. This usually only happens on very hard to
	 * read and decode barcodes, so decoding the normal barcodes is not affected by
	 * this.
	 *
	 * @param erasureArray         contains the indexes of erasures
	 * @param ambiguousIndexes     array with the indexes that have more than one
	 *                             most likely value
	 * @param ambiguousIndexValues two dimensional array that contains the ambiguous
	 *                             values. The first dimension must be the same
	 *                             length as the ambiguousIndexes array
	 */
	private static DecoderResult createDecoderResultFromAmbiguousValues(int ecLevel, int[] codewords,
			int[] erasureArray, int[] ambiguousIndexes, int[][] ambiguousIndexValues)
	...
	private static int getStartColumn(DetectionResult detectionResult, int barcodeColumn, int imageRow,
		...
		while (isValidBarcodeColumn(detectionResult, barcodeColumn - offset)) {
			...
			for (Codeword previousRowCodeword : detectionResult.getDetectionResultColumn(barcodeColumn)
					.getCodewords()) {
	...
	private static Codeword detectCodeword(BitMatrix image, int minColumn, int maxColumn, boolean leftToRight,
		...
		// we usually know fairly exact now how long a codeword is. We should provide
		// minimum and maximum expected length
		// and try to adjust the read pixels, e.g. remove single pixel errors or try to
		// cut off exceeding pixels.
		// min and maxCodewordWidth should not be used as they are calculated for the
		// whole barcode an can be inaccurate
		...
		// use start (and maybe stop pattern) to determine if black bars are wider than
		// white bars. If so, adjust.
		...
		// We could also use the width of surrounding codewords for more accurate
		// results, but this seems
		// sufficient for now
		if (!checkCodewordSkew(codewordBitCount, minCodewordWidth, maxCodewordWidth)) {
			// We could try to use the startX and endX position of the codeword in the same
			// column in the previous row,
			// create the bit count from it and normalize it to 8. This would help with
			// single pixel errors.
			return null;
		}
	...
	private static int[] getModuleBitCount(BitMatrix image, int minColumn, int maxColumn, boolean leftToRight,
		...
		if (moduleNumber == moduleBitCount.length || ((imageColumn == (leftToRight ? maxColumn : minColumn))
	...
	private static int adjustCodewordStartColumn(BitMatrix image, int minColumn, int maxColumn, boolean leftToRight,
			int codewordStartColumn, int imageRow) {
		int correctedStartColumn = codewordStartColumn;
		int increment = leftToRight ? -1 : 1;
		// there should be no black pixels before the start column. If there are, then
	 ...
	 * @param numECCodewords number of error correction codewords that are available
	 *                       in codewords
	 * @throws ChecksumException if error correction fails
	 */
	private static int correctErrors(int[] codewords, int[] erasures, int numECCodewords) throws ChecksumException {
		if (erasures != null && erasures.length > numECCodewords / 2 + MAX_ERRORS || numECCodewords < 0
	...
	private static void verifyCodewordCount(int[] codewords, int numECCodewords) throws FormatException {
		...
		// The first codeword, the Symbol Length Descriptor, shall always encode the
		// total number of data
		// codewords in the symbol, including the Symbol Length Descriptor itself, data
		// codewords and pad
		...
		if (numberOfCodewords == 0) {
			// Reset to the length of the array - 8 (Allow for at least level 3 Error
			// Correction (8 Error Codewords)
	...
	public static String toString(BarcodeValue[][] barcodeMatrix) {
		...
		for (int row = 0; row < barcodeMatrix.length; row++) {
			sb.append("Row ").append(row >= 10 ? "" + row : " " + row).append(":");
			...
			for (int column = 0; column < barcodeMatrix[row].length; column++) {
				...
				if (barcodeValue.getValue().length == 0) {
					...
					int i = barcodeValue.getValue()[0];
					int j = barcodeValue.getConfidence(barcodeValue.getValue()[0]);
					sb.append(i >= 1000 ? "" + i : i >= 100 ? " " + i : i >= 10 ? "  " + i : "   " + i).append("(")
							.append(j >= 10 ? "" + i : " " + i).append(")");
