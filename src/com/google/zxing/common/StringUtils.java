...
public final class StringUtils {
	...
	/**
	 * TODO Get platform default encoding. UTF-8 chosen because it is supported by
	 * all modern platforms but should be less efficient than using the true native
	 * encoding
	 */
	private static final String PLATFORM_DEFAULT_ENCODING = "UTF-8"; // Charset.defaultCharset().name();
	...
	public static String guessEncoding(byte[] bytes, Map<DecodeHintType, ?> hints) {
		...
		// evidence it can't be UTF-8), done
		if (canBeUTF8 && (utf8bom || utf2BytesChars + utf3BytesChars + utf4BytesChars > 0)) {
			return UTF8;
		}
		// Easy -- if assuming Shift_JIS or >= 3 valid consecutive not-ascii characters
		// (and no evidence it can't be), done
		if (canBeShiftJIS
		...
		// Otherwise, try in order ISO-8859-1, Shift JIS, UTF-8 and fall back to default
		// platform encoding
		if (canBeISO88591) {
			return ISO88591;
		}
