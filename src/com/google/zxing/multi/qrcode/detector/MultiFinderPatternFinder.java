...
final class MultiFinderPatternFinder extends FinderPatternFinder {
	...
	/**
	 * More or less arbitrary cutoff point for determining if two finder patterns
	 * might belong to the same code if they differ less than
	...
	/**
	 * More or less arbitrary cutoff point for determining if two finder patterns
	 * might belong to the same code if they differ less than DIFF_MODSIZE_CUTOFF
	 ...
	 *         "best" are those that have been detected at least 2 times, and whose
	 *         module size differs from the average among those patterns the least
	...
	private FinderPattern[][] selectMultipleBestPatterns() throws NotFoundException {
		 ...
		 * feature similar module sizes - are placed in a distance so the estimated
		 * module count is within the QR specification - have similar distance between
		 * upper left/right and left top/bottom finder patterns - form a triangle with
		 * 90° angle (checked by comparing top right/bottom left distance with
		 * pythagoras)
		 *
		 * Note: we allow each point to be used for more than one code region: this
		 * might seem counterintuitive at first, but the performance penalty is not that
		 * big. At this point, we cannot make a good quality decision whether the three
		 * finders actually represent a QR code, or are just by chance layouted so it
		 * looks like there might be a QR code there. So, if the layout seems right,
		 * lets have the decoder try to decode.
		 */
		...
		for (int i1 = 0; i1 < (size - 2); i1++) {
			...
			for (int i2 = i1 + 1; i2 < (size - 1); i2++) {
				...
				if (vModSize12A > DIFF_MODSIZE_CUTOFF && vModSize12 >= DIFF_MODSIZE_CUTOFF_PERCENT) {
					// break, since elements are ordered by the module size deviation there cannot
					// be
				...
				for (int i3 = i2 + 1; i3 < size; i3++) {
					...
					// Calculate the distances: a = topleft-bottomleft, b=topleft-topright, c =
					// diagonal
	...
	public FinderPatternInfo[] findMulti(Map<DecodeHintType, ?> hints) throws NotFoundException {
		...
		// the smallest
		// number of pixels the center could be, so skip this often. When trying harder,
		// look for all
