...
public class FinderPatternFinder {
	...
	/**
	 * Hold the required distance between finder patterns, if any. 
	 * @since 2019.08.20
	 */
	private double[] distanceBetweenFinders;
	private boolean useCheckFullFinderPattern;
	...
	final FinderPatternInfo find(Map<DecodeHintType, ?> hints) throws NotFoundException {
		...
		if (hints != null) {
			...
			if (hints.containsKey(DecodeHintType.TRY_HARDER)) {
				o = hints.get(DecodeHintType.TRY_HARDER);
			...
			// Added distance check @since 2019.08.20
			distanceBetweenFinders = hints.containsKey(DecodeHintType.FINDER_PATTERN_DISTANCE_APPROX_RATIO) ? 
					(double[]) hints.get(DecodeHintType.FINDER_PATTERN_DISTANCE_APPROX_RATIO) : 
					null;
			useCheckFullFinderPattern = hints.containsKey(DecodeHintType.QR_CODE_CHECK_FULL_FINDER_PATTERN);
			Arrays.sort(distanceBetweenFinders);
		}
	 ...
	 * @param centerJ center of the section that appears to cross a finder
	 * pattern
	 ...
	 * real center of the alignment pattern.</p>
	 */
	...
	protected final boolean handlePossibleCenter(int[] stateCount, int i, int j) {
		...
		if (!Float.isNaN(centerI)) {
			...
			if (!Float.isNaN(centerJ) && crossCheckDiagonal((int) centerI, (int) centerJ)) {
				...
				if (useCheckFullFinderPattern) {
					if (!checkIfCompletePattern(new FinderPattern(centerI, centerJ, estimatedModuleSize))) {
						return false;
					}
				}
				boolean found = false;
	 ...
	 * certain point farther down in the image.
	 */
	...
	/**
	 * @return true if we have found at least 3 finder patterns that have been
	 * detected at least {@link #CENTER_QUORUM} times each, and, the estimated
	 * module size of the candidates is "pretty similar"
	 * 
	 * @since 2019.8.20 I faced a bug with misidentified finder pattern. Added 
	 * additional check which requires the three distances to be approximately  
	 * the same
	 * @author Roy Wang
	 */
	private boolean haveMultiplyConfirmedCenters() {
		...
		// Added distance check @since 2019.08.20
		if (distanceBetweenFinders != null) {
			double d1 = squaredDistance(possibleCenters.get(0), possibleCenters.get(1));
			double d2 = squaredDistance(possibleCenters.get(0), possibleCenters.get(2));
			double d3 = squaredDistance(possibleCenters.get(1), possibleCenters.get(2));
			double[] actualDist = new double[] {
				d1, 
				d2, 
				d3,
			};
			Arrays.sort(actualDist);
			return Arrays.equals(actualDist, distanceBetweenFinders);
		}
	...
	/**
	 * Check if the QR code finder is actually a true finder. 
	 * @param pattern
	 * @return 
	 */
	private boolean checkIfCompletePattern(FinderPattern pattern) {
		float x = pattern.getX();
		float y = pattern.getY();
		final float m = pattern.getEstimatedModuleSize();
		...
		for (int j = 0; j < 7; j++) {
			...
			for (int i = 0; i < 7; i++) {
				x = x < 0 ? 0 : x >= w ? w - 1 : x;
				if (image.get((int) x, (int) y) != FinderPattern.getExpectedDot(i, j)) {
					return false;
				}
			}
		}
	 ...
	 * The "best" are those have similar module size and form a shape closer to
	...
	/**
	 * <p>
	 * Orders by {@link FinderPattern#getEstimatedModuleSize()}</p>
	 */
	private static final class EstimatedModuleComparator implements Comparator<FinderPattern>, Serializable {
		// Generated

		private static final long serialVersionUID = 6717908871926505205L;
