...
public final class VINResultParser extends ResultParser {
  ...
  private static final RE IOQ = new RE("[IOQ]");
  private static final RE AZ09 = new RE("[A-Z0-9]{17}");
  ...
  public VINParsedResult parse(Result result) {
    ...
    //IOQ.match(rawText);
    rawText = IOQ.subst(rawText, "").trim();
    //rawText = IOQ.matcher(rawText).replaceAll("").trim();
    boolean shouldBeTrue = AZ09.match(rawText);
    //    if (!AZ09.matcher(rawText).matches()) {
    if (!shouldBeTrue || AZ09.getParenLength(0) != rawText.length()) {
      return null;
    }
    try {
