...
public abstract class ResultParser {
	...
	private static final ResultParser[] PARSERS = { new BookmarkDoCoMoResultParser(),
			...
			new URIResultParser(), new ISBNResultParser(), new ProductResultParser(), new ExpandedProductResultParser(),
			new VINResultParser(), };

	// Added ^ and $ to indicate whole string
	...
	protected static boolean isStringOfDigits(CharSequence value, int length) {
		return value != null && length > 0 && length == value.length() && DIGITS.match(value.toString());
	}
	...
	protected static boolean isSubstringOfDigits(CharSequence value, int offset, int length) {
		...
		return value.length() >= max && DIGITS.match(value.subSequence(offset, max).toString());
	}
	...
	private static void appendKeyValue(CharSequence keyValue, Map<String, String> result) {
		// Need to split into two by the first equal
		boolean shouldBeTrue = EQUALS.match(keyValue.toString());
		if (!shouldBeTrue) {
			return;
		}
		int s1 = EQUALS.getParenStart(0);
		int s2 = EQUALS.getParenEnd(0);
		String[] keyValueTokens = new String[] { keyValue.subSequence(0, s1).toString(),
				keyValue.subSequence(s2, keyValue.length()).toString() };
	...
	static String urlDecode(String encoded) {
		return Util.decode(encoded, "UTF-8", true);
